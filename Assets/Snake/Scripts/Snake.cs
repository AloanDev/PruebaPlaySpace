...
public class Snake : MonoBehaviour {
    ...
    public GameObject effectFood, effectCoin;
    ...
    private void HandleGridMovement() {
        ...
        if (gridMoveTimer >= gridMoveTimerMax) {
            ...
            if (snakeAteFood) {
                ...
                Instantiate(effectFood, new Vector2(localPosition.x, localPosition.y), Quaternion.identity);
                snakeBodySize++;
                CreateSnakeBodyPart();
                SoundManager.PlaySound(SoundManager.Sound.SnakeEat);
            }
            bool snakeAteCoin = levelGrid.TrySnakeEatCoin(gridPosition);
            if (snakeAteCoin) {
                // Snake ate food, grow body
                var localPosition = transform.localPosition;
                Instantiate(effectCoin, new Vector2(localPosition.x, localPosition.y), Quaternion.identity);
                snakeBodySize = snakeBodySize + 3;
                CreateSnake3BodyPart();
                SoundManager.PlaySound(SoundManager.Sound.SnakeEat);
            }
    ...
    private void CreateSnake3BodyPart() {
        snakeBodyPartList.Add(new SnakeBodyPart(snakeBodyPartList.Count));
        snakeBodyPartList.Add(new SnakeBodyPart(snakeBodyPartList.Count));
        snakeBodyPartList.Add(new SnakeBodyPart(snakeBodyPartList.Count));
